...
//		echo "POSTED CAT: ".$category; // -- debug --
	  ...
	  if (isset($_POST['modifyfile']))
	  {
		...
		if ($result)
		{
			$operation_result="<div class='ok'>Файл успешно ".$message."</div>";
		}
		else
		{
			$operation_result="<div class='error'>Ошибка при ".$message."ии файла ".mysql_error()."</div>";
		}
		unset ($querystart);
		unset ($queryend);
		unset ($create_date);
...
//			$ref = returnfileSuffix($file['image']);
		...
		if ($result)
		{
			$operation_result="<div class='ok'>Файл успешно удален</div>";
		}
		else
		{
			$operation_result="<div class='error'>Ошибка при удалении файла</div>";
		}
	}
	...
	if (isset($_POST['addcat']))
	{
		...
		if ($result)
		{
			$operation_result="<div class='ok'>Категория &quot;".$_POST['title']."&quot; добавлена</div>";
		}
		else
		{
			$operation_result="<div class='error'>Ошибка при добавлении категории &quot;".$_POST['title']."&quot;</div>";
		}
	}
	...
	if (isset($_POST['modifycat']))
	{
		...
		if ($result)
		{
			$operation_result="<div class='ok'>Категория изменеена</div>";
		}
		else
		{
			$operation_result="<div class='error'>Ошибка при изменении категории</div>";
		}
	}
	...
	if (isset($_POST['delcat']))
	{
		...
		if ($result)
		{
			$operation_result="<div class='ok'>Категория &quot;".$cat['title']."&quot; удалена</div>";
		}
		else
		{
			$operation_result="<div class='error'>Ошибка при удалении категории &quot;".$_POST['title']."&quot;</div>";
		}
	}
?>
...
<?
  ...
  if (!$_GET['act'])
  {
	...
	echo "Сегодня скачиваний : <strong>".$downloads_today."</strong><br><br>";
	echo "Всего скачиваний : <strong>".$downloads_total."</strong><br><br>";
	switch ($size)
	{
	...
	echo "Общий размер размещённых файлов : <strong>".$size." ".$units."</strong>";
	echo "</div>";
  }
  else
  {
...
<?
		...
		{
			echo " <option value=''>-- Выберите программу из списка --</option>";
		}
		...
		echo "</td></tr>";
		echo "<tr><td align='center'>";
		echo "<input name='modifyfile' type='submit' value='  Открыть для изменения  '>";
		echo "</form>";
		echo "</td></tr>";
		echo "</table>";
		echo "<hr>";		
		...
		echo "<table cellpadding='3' cellspacing='3' border='0' align='center'>";
		echo "<tr><td align='center'>";
		echo "Выберите программу из списка";
		echo "</td></tr>";
		echo "<tr><td>";
		...
		echo "</td></tr>";
		echo "<tr><td align='center'>";
		echo "<input name='delfile' type='submit' value='  Удалить  '>";
		echo "</form>";
		echo "</td></tr>";
		echo "</table>";
		...
		echo "<table cellpadding='3' cellspacing='3' border='0' align='center'>";
		echo "<tr><td align='center'>";
		echo "Добавить родительскую категорию";
		echo "</td></tr>";
		echo "<tr><td>";
		...
		echo "</select>";
		echo "<tr><td align='center'>";
		echo "Новое название категории: <input name='title' type='text' size='45' maxlength='250'>";
		echo "</td></tr>";
		echo "<tr><td align='center'>";
		echo "<input name='modifycat' type='submit' value='  Изменить  '>";
		echo "</td></tr>";
		echo "</form>";
		echo "</table>";
		break;
		...
		echo "</td></tr>";
		echo "<tr><td align='center'>";
		echo "<input name='delcat' type='submit' value='  Удалить  '>";
		echo "</form>";
		echo "</td></tr>";
		echo "</table>";
		break;
		...
		if ($_GET['rmcomm_id'])
		{
			...
			if ($result)
			{
				$operation_result="<div class='ok'>Комментарий успешко удалён</div>";
			}
			else
			{
				$operation_result="<div class='error'>Ошибка при удалении комментария</div>";
			}
		}
		...
		echo "</td></tr>";
		echo "<tr><td align='center'>";
		echo "<input name='list_comments' type='submit' value='  Отобразить комментарии  '>";
		echo "</form>";
		echo "</td></tr>";
		echo "</table>";
		...
		echo "<br>";
		echo "<div align='center'><a href='main.php?act=".$_GET['act']."&mode=listall'>Показать все комментарии</a></div><br>";
		if ($operation_result)
		{
		...
		if (mysql_num_rows($result))
		{
			...
			echo "<td bgcolor='#BCC7CF'><div class='comment_header_text'>Комментарии пользователей</div></td>";
			echo "</tr>";
			echo "</table>";			
		}
		...
		while ($comment = mysql_fetch_array($result))
		{
			...
			echo "<tr><td align='left'><input name='select[".$row."]' type='checkbox' value='".$comment['id']."'>&nbsp;&nbsp;<font class='comment_date'>".date("d/m/Y, H:i", $comment['datetime']).",</font> <font class='comment_added_by'>".$comment['added_by']."</font> <strong>(".$comment['ip_address'].")</strong>, ID программы - <a href='../index.php?id=".$comment['file_id']."'><strong>".$comment['file_id']."</strong></a> :<a href='main.php?act=".$_GET['act'];
			$row++;
			if ($_GET['mode'])
			{
				echo "&mode=".$_GET['mode'];
			}
			echo "&file_id=".$file_id."&rmcomm_id=".$comment['id']."'<strong><font class='error'>[Удалить]</font></strong></td></tr>";
			echo "<tr><td align='left'><div class='comment_text'>".wordwrap($comment['text'], 80, "<br />\n", true)."</div></td></tr>";
			echo "<tr height='3'><td></td></tr>";
		}
		echo "</table>";
		echo "<input name='removegroup' type='submit' value='Удалить' title='Удалить выделенные комментарии'>";
		echo "</form>";
		break;
	}
  }
